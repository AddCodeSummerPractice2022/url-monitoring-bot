...
from handlers import Notification, addSite, showSpisok
...
async def sql_add_site():
    ...
    cur.execute('INSERT INTO Log(ID, Site_url, Status, Ping, Check_Date) VALUES (?, ?, ?, ?, ?)', (addSite.sites_db.id, addSite.sites_db.site_url, addSite.sites_db.status, addSite.sites_db.ping, addSite.sites_db.check_date))
    DataBase.commit()

async def sql_update_site(status: str, ping: str, check_date: str, uid: str):
    cur.execute('UPDATE Sites SET Status=?, Ping=?, Check_Date=? WHERE ID=?', (status, ping, check_date, uid))
    DataBase.commit()

# Функция добавления сайта в таблицу Log ##########
async def sql_add_sitelog(uid: str, url: str, status: str, ping: str, check_date: str):
    cur.execute('INSERT INTO Log(ID, Site_url, Status, Ping, Check_Date) VALUES (?, ?, ?, ?, ?)', (uid, url, status, ping, check_date))
    DataBase.commit()
...
def sql_get_url(tg_user_id : str):
    cur.execute('SELECT Sites.Site_url FROM Sites INNER JOIN Connections ON Sites.ID=Connections.UID_Site INNER JOIN USERS ON Connections.UID_User=Users.ID WHERE Users.tg_user_id=?', (tg_user_id,))
    url = cur.fetchall()
    return url

def sql_get_uid(tg_user_id : str):
    cur.execute('SELECT Sites.ID FROM Sites INNER JOIN Connections ON Sites.ID=Connections.UID_Site INNER JOIN USERS ON Connections.UID_User=Users.ID WHERE Users.tg_user_id=?', (tg_user_id,))
    uid = cur.fetchall()
    return uid
...
def sql_get_last_date(UID_Site : str):
    cur.execute('SELECT Check_Date FROM Log WHERE ID = ? ORDER BY Check_date DESC', (UID_Site,))
    check_date = str(cur.fetchone())
