...
def sql_start_tables():
    ...
    DataBase.execute(
        'CREATE TABLE IF NOT EXISTS Sites(ID TEXT PRIMARY KEY, Site_url TEXT, Status TEXT, Ping TEXT, Check_Date TEXT)')
    DataBase.execute(
        'CREATE TABLE IF NOT EXISTS Connections(ID TEXT, UID_User TEXT, UID_Site TEXT, FOREIGN KEY(UID_Site) '
        'REFERENCES Sites(ID) ON DELETE CASCADE, FOREIGN KEY(UID_User) REFERENCES Users(ID) ON DELETE CASCADE)')
    DataBase.commit()


# Функция добавления пользователя в таблицу Users
async def sql_add_user():
    cur.execute('INSERT INTO Users(tg_user_id, ID) VALUES (?, ?)',
                (addSite.users_db.tg_user_id, addSite.users_db.UID_user))
    DataBase.commit()


# Функция добавления сайта в таблицу Sites
async def sql_add_site():
    cur.execute('INSERT INTO Sites(ID, Site_url, Status, Ping, Check_Date) VALUES (?, ?, ?, ?, ?)', (
        addSite.sites_db.id, addSite.sites_db.site_url, addSite.sites_db.status, addSite.sites_db.ping,
        addSite.sites_db.check_date))
    DataBase.commit()
...
async def sql_add_conns():
    cur.execute('PRAGMA foreign_keys = on')
    cur.execute('INSERT INTO Connections(ID, UID_User, UID_Site) VALUES (?, ?, ?)',
                (addSite.users_db.tg_user_id, addSite.users_db.UID_user, addSite.sites_db.id))
    DataBase.commit()
...
def sql_get_spisok(tg_user_id: str):
    cur.execute(
        'SELECT Sites.Site_url, Sites.Status, Sites.Ping FROM Sites INNER JOIN Connections ON '
        'Sites.ID=Connections.UID_Site INNER JOIN USERS ON Connections.UID_User=Users.ID WHERE Users.tg_user_id=?',
        (tg_user_id,))
    spisok = cur.fetchall()
    return spisok
...
def sql_remove_site(tg_user_id: str, number: int):
    ...
    cur.execute('DELETE FROM Connections WHERE UID_Site = ? and UID_User = ?',
                (spisok_of_UID_site[number - 1], spisok_of_UID_user[number - 1],))
